...
class FGlo(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.avg_pool(x).view(b, c)  # 保持batch和channel
...
class ContextGuidedBlock_Down(nn.Module):
    ...
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16):
        ...
        """
        super().__init__()
        self.conv1x1 = ConvBNPReLU(nIn, nOut, 3, 2)  #  size/2, channel: nIn--->nOut.kernel_size=3,stride=2,for downsample
...
class Context_Guided_Network(nn.Module):
    ...
    """
    def __init__(self, classes=19, M= 3, N= 21, dropout_flag = False):  # 这里N改成11，取中间层结果
        """
        args:
        ...
        #     self.classifier = nn.Sequential(nn.Dropout2d(0.1, False),Conv(256, classes, 1, 1))  #  kernel=1的卷积，相当于FCN，分多类
        # else:
    ...
    def forward(self, input):
        ...
        return output2_cat  # change output here
