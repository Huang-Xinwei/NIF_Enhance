...
from model_arch.fast_scnn import FastSCNN
...
class ColorFSNet(nn.Module):
    def __init__(self):
        super(ColorFSNet, self).__init__()
        self.FSNet = FastSCNN()
        self.dealFSNet()
        ...
        self.connect_sem = nn.Sequential( DoubleConvBlock(128, 128), DoubleConvBlock(128, 128) )
    
    def dealFSNet(self):
        pth_path = './models/fast_scnn_citys.pth'
        mol = torch.load(pth_path)
        for k in list(mol.keys()):
            if 'classifier' in k:
                del mol[k]                                                
        self.FSNet.load_state_dict(mol)
        for param in self.FSNet.parameters():
